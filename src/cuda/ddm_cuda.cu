...
#define CUFFTFCOMPLEX cufftComplex
...
/*!
    Transfer images on GPU and compute fft2 (single precision)
 */
template <typename T>
                         ...
                         unsigned long long pitch_nx)
{
    ...
    unsigned long long batch = (length - 1) / num_fft2 + 1;
    // compute fft2 normalizaton factor
    float norm_fact = 1.0 / sqrtf((float)(nx * ny));

    // ***Allocate device arrays
    // workspace
    float *d_workspace;
    gpuErrchk(cudaMalloc(&d_workspace, 2 * pitch_nx * ny * batch * sizeof(float)));
    // buffer (only allocate if T is not double)
    T *d_buff;
    ...
    int minGridSize;    // The minimum grid size needed to achieve the
                        // maximum occupancy for a full device launch
    gpuErrchk(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize_copy, copy_convert_single_kernel<T>, 0, 0));
    dim3 gridSize_copy(min(1ULL * maxGridSizeX, batch), min(1ULL * maxGridSizeY, height), 1);
    ...
    gpuErrchk(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize_scale, scale_array_single_kernel, 0, 0));
    // Round up according to array size
    gridSize_scale = min((ny * batch + blockSize_scale - 1) / blockSize_scale, 32ULL * numSMs);
    ...
    {
        // rezero workspace array
        gpuErrchk(cudaMemset(d_workspace, 0.0, 2 * pitch_nx * ny * batch * sizeof(float)));
        ...
        if (std::is_same<T, float>::value)
        {
            ...
            params.srcArray = NULL;
            params.srcPos = make_cudaPos(0, 0, ii * batch);
            params.srcPtr = make_cudaPitchedPtr((float *)h_in, width * sizeof(float), width, height);
            params.dstArray = NULL;
            params.dstPos = make_cudaPos(0, 0, 0);
            params.dstPtr = make_cudaPitchedPtr(d_workspace, 2 * pitch_nx * sizeof(float), 2 * pitch_nx, ny);
            params.extent = make_cudaExtent(width * sizeof(float), height, num_imgs_copy);
            params.kind = cudaMemcpyHostToDevice;
        ...
        else
        {
            ...
            // convert values of buffer into workspace
            copy_convert_single_kernel<<<gridSize_copy, blockSize_copy>>>(d_buff,
                                                                          d_workspace,
                                                                          width,
                                                                          height,
                                                                          batch,
        ...
        // ***Execute fft2 plan
        cufftSafeCall(cufftExecR2C(fft2_plan, d_workspace, (CUFFTFCOMPLEX *)d_workspace));
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());
        ...
        unsigned long long end = (ii + 1) * batch > length ? length * ny : (ii + 1) * ny * batch;
        // scale array
        scale_array_single_kernel<<<gridSize_scale, blockSize_scale>>>((float2 *)d_workspace,
                                                                       pitch_nx,
                                                                       _nx,
                                                                       norm_fact,
                                                                       end - start);
        ...
        // ***Copy values back to host
        gpuErrchk(cudaMemcpy2D((float2 *)h_out + _nx * start, _nx * sizeof(float2), (float2 *)d_workspace, pitch_nx * sizeof(float2), _nx * sizeof(float2), end - start, cudaMemcpyDeviceToHost));
    }
...
template void compute_fft2_single<u_int8_t>(const u_int8_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<int16_t>(const int16_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<u_int16_t>(const u_int16_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<int32_t>(const int32_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<u_int32_t>(const u_int32_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<int64_t>(const int64_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<u_int64_t>(const u_int64_t *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<float>(const float *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
template void compute_fft2_single<double>(const double *h_in, float *h_out, unsigned long long width, unsigned long long height, unsigned long long length, unsigned long long nx, unsigned long long ny, unsigned long long num_fft2, unsigned long long buff_pitch, unsigned long long pitch_nx);
...
/*!
    Compute image structure function using differences on the GPU (single precision)
 */
                                    ...
                                    unsigned long long pitch_t)
{
    ...
    // workspaces
    double *d_workspace1, *d_workspace2;
    unsigned long long workspace_size = max(pitch_q * length, chunk_size * pitch_t) * 2 * sizeof(float);
    gpuErrchk(cudaMalloc(&d_workspace1, workspace_size));
    gpuErrchk(cudaMalloc(&d_workspace2, workspace_size));
    // helper arrays
    unsigned int *d_lags;
    float2 *d_power_spec, *d_var;
    gpuErrchk(cudaMalloc(&d_lags, lags.size() * sizeof(unsigned int)));
    gpuErrchk(cudaMemcpy(d_lags, lags.data(), lags.size() * sizeof(unsigned int), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMalloc(&d_power_spec, chunk_size * sizeof(float2)));
    gpuErrchk(cudaMalloc(&d_var, chunk_size * sizeof(float2)));
    ...
    int blockSize_lc; // The launch configurator returned block size
    int gridSize_lc;  // The actual grid size needed, based on input size
    gpuErrchk(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize_lc, linear_combination_single_kernel, 0, 0));
    // Round up according to array size
    gridSize_lc = min((chunk_size + blockSize_lc - 1) / blockSize_lc, 32ULL * numSMs);
    ...
    for (unsigned long long chunk = 0; chunk < num_chunks; chunk++)
    {
        ...
        // to speed up transfer, use pitch_q
        gpuErrchk(cudaMemcpy2D(d_workspace2,
                               2 * pitch_q * sizeof(float),
                               h_in + 2 * q_start,
                               2 * _nx * ny * sizeof(float),
                               2 * curr_chunk_size * sizeof(float),
                               length,
                               cudaMemcpyHostToDevice));

        // ***Transpose array (d_workspace2 --> d_workspace1)
        transpose_complex_matrix_single_kernel<<<gridSize_tran1, blockSize_tran>>>((float2 *)d_workspace2,
                                                                                   pitch_q,
                                                                                   (float2 *)d_workspace1,
                                                                                   pitch_t,
                                                                                   curr_chunk_size,
        ...
        // ***Compute structure function using differences (d_workspace1 --> d_workspace2)
        structure_function_diff_single_kernel<<<gridSize_corr, blockSize_corr, smemSize>>>((float2 *)d_workspace1,
                                                                                           (float2 *)d_workspace2,
                                                                                           d_lags,
                                                                                           length,
                                                                                           lags.size(),
        ...
        // ***Compute power spectrum (d_workspace1 --> d_power_spect)
        average_power_spectrum_single_kernel<<<gridSize_red, blockSize_red, smemSize>>>((float2 *)d_workspace1,
                                                                                        d_power_spec,
                                                                                        length,
                                                                                        pitch_t,
                                                                                        curr_chunk_size);
        ...
        // ***Compute variance (d_workspace1 --> d_var)
        // compute average over time
        average_complex_single_kernel<<<gridSize_red, blockSize_red, smemSize2>>>((float2 *)d_workspace1,
                                                                                  d_var,
                                                                                  length,
                                                                                  pitch_t,
                                                                                  curr_chunk_size);
        ...
        // compute square modulus
        square_modulus_single_kernel<<<gridSize_lc, blockSize_lc>>>(d_var,
                                                                    1,
                                                                    1,
                                                                    curr_chunk_size);
        ...
        // linear combination (d_var = d_power_spec - d_var)
        linear_combination_single_kernel<<<gridSize_lc, blockSize_lc>>>(d_var,
                                                                        d_power_spec,
                                                                        make_float2(1.0, 0.0),
                                                                        d_var,
                                                                        make_float2(-1.0, 0.0),
        ...
        // ***Transpose array (d_workspace2 --> d_workspace1)
        transpose_complex_matrix_single_kernel<<<gridSize_tran2, blockSize_tran>>>((float2 *)d_workspace2,
                                                                                   pitch_t,
                                                                                   (float2 *)d_workspace1,
                                                                                   pitch_q,
                                                                                   lags.size(),
        ...
        // ***Copy values from device to host
        // elements are treated as complex floats
        // to speed up transfer, use pitch_q
        gpuErrchk(cudaMemcpy2D(h_in + 2 * q_start,
                               2 * _nx * ny * sizeof(float),
                               d_workspace1,
                               ...
                               cudaMemcpyDeviceToHost));
        // copy power spectrum
        gpuErrchk(cudaMemcpy((float2 *)h_in + (unsigned long long)(lags.size()) * _nx * ny + q_start,
                             d_power_spec,
                             curr_chunk_size * sizeof(float2),
                             cudaMemcpyDeviceToHost));
        // copy variance
        gpuErrchk(cudaMemcpy((float2 *)h_in + (unsigned long long)(lags.size() + 1) * _nx * ny + q_start,
                             d_var,
                             curr_chunk_size * sizeof(float2),
                             cudaMemcpyDeviceToHost));
    }
                            ...
                            unsigned long long pitch_fs)
{
    ...
    gpuErrchk(cudaMalloc(&d_workspace1, pitch_fs * ny * chunk_size * 2 * sizeof(float)));
    gpuErrchk(cudaMalloc(&d_workspace2, pitch_fs * ny * chunk_size * 2 * sizeof(float)));
    ...
    for (unsigned long long chunk = 0; chunk < num_fullshift; chunk++)
    {
        ...
        // ***Make full power spectrum (workspace1 --> workspace2)
        make_full_powspec_single_kernel<<<gridSize_full, blockSize_full>>>((float2 *)d_workspace1,
                                                                           pitch_fs,
                                                                           d_workspace2,
                                                                           2 * pitch_fs,
                                                                           _nx,
        ...
        // ***Shift power spectrum (workspace2 --> workspace1)
        shift_powspec_single_kernel<<<gridSize_shift, blockSize_full>>>(d_workspace2,
                                                                        2 * pitch_fs,
                                                                        d_workspace1,
                                                                        2 * pitch_fs,
                                                                        nx,
...
/*!
    Compute image structure function using the WK theorem on the GPU (single precision)
 */
                                   ...
                                   unsigned long long pitch_nt)
{
    ...
    // To speed up memory trasfer and meet the alignment requirements for coalescing,
    // we space each subarray by pitch1 (over q) or pitch2 (over t)
    gpuErrchk(cudaMalloc(&d_workspace1, chunk_size * pitch_nt * 2 * sizeof(float)));
    unsigned long long workspace2_size = max(chunk_size * pitch_t, pitch_q * length);
    gpuErrchk(cudaMalloc(&d_workspace2, workspace2_size * 2 * sizeof(float)));
    ...
    int blockSize_sqmod; // The launch configurator returned block size
    int gridSize_sqmod;  // The actual grid size needed, based on input size
    gpuErrchk(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize_sqmod, square_modulus_single_kernel, 0, 0));
    // Round up according to array size
    gridSize_sqmod = min((chunk_size + blockSize_sqmod - 1) / blockSize_sqmod, 32ULL * numSMs);
    ...
    int blockSize_final; // The launch configurator returned block size
    int gridSize_final;  // The actual grid size needed, based on input size
    gpuErrchk(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize_final, linear_combination_final_single_kernel, 0, 0));
    // Round up according to array size
    gridSize_final = min(chunk_size, 32ULL * numSMs);
    ...
    int blockSize_copy; // The launch configurator returned block size
    int gridSize_copy;  // The actual grid size needed, based on input size
    gpuErrchk(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize_copy, copy_selected_lags_single_kernel, 0, 0));
    // Round up according to array size
    gridSize_copy = min(chunk_size, 32ULL * numSMs);
    ...
    for (unsigned long long chunk = 0; chunk < num_chunks; chunk++)
    {
        ...
        // ***Zero-out elements of workspace1 and workspace2 device arrays
        gpuErrchk(cudaMemset2D(d_workspace1, 2 * pitch_nt * sizeof(float), 0.0, 2 * nt * sizeof(float), curr_chunk_size));
        gpuErrchk(cudaMemset(d_workspace2, 0.0, 2 * workspace2_size * sizeof(float)));

        // ***Copy values from host buffer to device workspace2 with pitch_q
        unsigned long long offset = 2 * q_start;                    // host source array offset
        unsigned long long spitch = 2 * (_nx * ny) * sizeof(float); // host source array pitch
        unsigned long long dpitch = 2 * pitch_q * sizeof(float);    // device destination array pitch
        gpuErrchk(cudaMemcpy2D(d_workspace2,
                               dpitch,
                               h_in + offset,
                               spitch,
        ...
        // ***Copy values ({d_workspace1; pitch_nt} --> {d_workspace2; pitch_t})
        dpitch = 2 * pitch_t * sizeof(float);
        spitch = 2 * pitch_nt * sizeof(float);
        gpuErrchk(cudaMemcpy2D(d_workspace2,
                               dpitch,
                               d_workspace1,
                               spitch,
        ...
        // +++ FFT PART +++
        // ***Do fft (d_workspace1 --> d_workspace1)
        cufftSafeCall(cufftExecC2C(fft_plan, (CUFFTFCOMPLEX *)d_workspace1, (CUFFTFCOMPLEX *)d_workspace1, CUFFT_FORWARD));
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());

        // ***Compute square modulus (d_workspace1 --> d_workspace1)
        square_modulus_single_kernel<<<gridSize_sqmod, blockSize_sqmod>>>((float2 *)d_workspace1,
                                                                          nt,
                                                                          pitch_nt,
                                                                          curr_chunk_size);
        gpuErrchk(cudaPeekAtLastError());
        ...
        // ***Scale fft part (d_workspace1 --> d_workspace1)
        scale_array_single_kernel<<<gridSize_scale, blockSize_scale>>>((CUFFTFCOMPLEX *)d_workspace1,
                                                                       pitch_nt,
                                                                       nt,
                                                                       1.0 / (float)nt,
                                                                       curr_chunk_size);
        ...
        // scale power spectrum (d_power_spec = d_power_spec * 1/length + 0)
        linear_combination_single_kernel<<<gridSize_lc, blockSize_lc>>>(d_power_spec,
                                                                        d_power_spec,
                                                                        make_float2(1.0 / (float)length, 0.0),
                                                                        d_power_spec,
                                                                        make_float2(0.0, 0.0),
                                                                        curr_chunk_size);
        ...
        linear_combination_single_kernel<<<gridSize_lc, blockSize_lc>>>(d_var,
                                                                        ...
                                                                        make_float2(1.0, 0.0),
                                                                        d_var,
                                                                        make_float2(-1.0 / (float)(length * length), 0.0),
                                                                        curr_chunk_size);

        // +++ CUMULATIVE SUM PART +++
        // ***Compute square modulus (d_workspace2 --> d_workspace2)
        square_modulus_single_kernel<<<gridSize_sqmod, blockSize_sqmod>>>((float2 *)d_workspace2,
                                                                          length,
                                                                          pitch_t,
                                                                          curr_chunk_size);
        ...
        // ***Copy the value into the imaginary part of the opposite (with respect to time) element (d_workspace2 --> d_workspace2)
        real2imagopposite_single_kernel<<<gridSize_sqmod, blockSize_sqmod>>>((CUFFTFCOMPLEX *)d_workspace2,
                                                                             length,
                                                                             pitch_t,
                                                                             curr_chunk_size);
        gpuErrchk(cudaPeekAtLastError());
        ...
        //  ***Linearly combine the two parts (workspace1 + workspace2 --> workspace2)
        linear_combination_final_single_kernel<<<gridSize_final, blockSize_final>>>((float2 *)d_workspace2,
                                                                                    pitch_t,
                                                                                    (float2 *)d_workspace1,
                                                                                    pitch_nt,
                                                                                    (float2 *)d_workspace2,
        ...
        // ***Keep only selected lags ({d_workspace2; pitch_t} --> {d_workspace1; pitch_t})
        copy_selected_lags_single_kernel<<<gridSize_copy, blockSize_copy>>>((float2 *)d_workspace2,
                                                                            (float2 *)d_workspace1,
                                                                            d_lags,
                                                                            lags.size(),
                                                                            pitch_t,
        ...
        // ***Transpose complex matrix ({d_workspace1; pitch_t} --> {d_workspace2; pitch_q})
        transpose_complex_matrix_single_kernel<<<gridSize_tran2, blockSize_tran>>>((float2 *)d_workspace1,
                                                                                   pitch_t,
                                                                                   (float2 *)d_workspace2,
                                                                                   pitch_q,
                                                                                   lags.size(),
        ...
        // to speed up transfer, use pitch_q
        offset = 2 * q_start;                    // host destination array offset
        spitch = 2 * pitch_q * sizeof(float);    // host source array pitch
        dpitch = 2 * (_nx * ny) * sizeof(float); // device destination array pitch
        gpuErrchk(cudaMemcpy2D(h_in + offset,
                               dpitch,
                               d_workspace2,
                               spitch,
