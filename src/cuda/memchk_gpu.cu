    ...
    */
    ...
    while (true)
    {
        ...
        if (cufft_res == CUFFT_SUCCESS)
        {
            ...
            mem_required += chunk_size * pitch_nt * 16ULL;

            // Add the required memory for the workspace2 array
            mem_required += max(pitch_q * length, chunk_size * pitch_t) * 16ULL;

            // Add the required memory for the prefix sum buffer
            mem_required += 2ULL * length < 1024ULL ? 2ULL * chunk_size * 8ULL : chunk_size * (1ULL + 2ULL * ((2ULL * length) / 1024ULL)) * 8ULL;
