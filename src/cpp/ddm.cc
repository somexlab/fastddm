                            ...
                            unsigned long long ny)
{
    ...
    // ***Execute fft2 plan
    fftwf_execute(fft2_plan);

    // ***Normalize fft2
    // use sqrt(num_pixels) for Parseval theorem
    float norm_fact = sqrtf((float)(nx * ny));
    for (unsigned long long ii = 0; ii < 2 * _nx * ny * length; ii++)
    {
        p_out[ii] /= norm_fact;
    }
                            ...
                            unsigned long long chunk_size)
{
    ...
    // ***Allocate workspace
    vector<float> workspace(2 * chunk_size * nt);
    ...
    for (unsigned long long i = 0; i < (_nx * ny - 1) / chunk_size + 1; i++)
    {
        ...
        for (unsigned long long q = 0; q < chunk_size; q++)
        {
            tmpAvg[q] = workspace[2 * q * nt] / (float)(length * length);
        }
