                                             ...
                                             py::array_t<Scalar, py::array::c_style> window)
{
    ...
    FFTW_PLAN fft2_plan = fft2_create_plan(p_out, nx, ny, length);
    ...
    for (unsigned long long q = 0; q < _nx * ny; q++) {
        ...
        for (unsigned long long _dt = 0; _dt < lags.size(); _dt++) {
            ...
            for (unsigned long long t = 0; t < length - dt; t++) {
                // compute the power spectrum of the difference of pixel at time t and time t+dt,
                // i.e.
        ...
        tmp2 /= (double)length;
        tmp[lags.size() + 1]
            = tmp[lags.size()] - tmp[lags.size() + 1] * tmp[lags.size() + 1] - tmp2 * tmp2;

        // copy the values back in the vector
        copy_vec_with_stride(tmp, p_out, 2 * q, 2 * (_nx * ny));
    }

    // Convert raw output to shifted structure function
    make_shifted_isf(p_out, nx, ny, lags.size() + 2);
...
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<uint8_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<int16_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<uint16_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<int32_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<uint32_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<int64_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<uint64_t, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<float, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_diff(py::array_t<double, py::array::c_style> img_seq,
                             vector<unsigned int> lags,
                             unsigned long long nx,
                             unsigned long long ny,
                             py::array_t<Scalar, py::array::c_style> window);
                                            ...
                                            py::array_t<Scalar, py::array::c_style> window)
{
    ...
    // ***Create the fft plan
    fftw_plan fft_plan = fft_create_plan(workspace, nt, chunk_size);
    ...
    for (unsigned long long i = 0; i < (_nx * ny - 1) / chunk_size + 1; i++) {
        ...
        for (unsigned long long q = 0; q < chunk_size; q++) {
            for (unsigned long long t = 0; t < length; t++) {
                workspace[2 * (q * nt + t)]
                    = p_out[2 * (t * _nx * ny + i * chunk_size + q)] / norm_fact_2; // real
                workspace[2 * (q * nt + t) + 1]
                    = p_out[2 * (t * _nx * ny + i * chunk_size + q) + 1] / norm_fact_2; // imag
            }
        ...
        for (unsigned long long t = 0; t < length; t++) {
            ...
            if ((idx < lags.size()) && (length - t - 1 == lags[lags.size() - 1 - idx])) {
                for (unsigned long long q = 0; q < chunk_size; ++q) {
                    ...
                    workspace[2 * (q * nt + lags[lags.size() - 1 - idx])] = tmp[q]
                        - 2.0
                            * workspace[2
                                        * (q * nt + lags[lags.size() - 1 - idx])]; // / (double)nt;
                    // finally, normalize output
                    workspace[2 * (q * nt + lags[lags.size() - 1 - idx])]
                        /= (double)(length - lags[lags.size() - 1 - idx]);
                }
                idx++;
            }
        }
        ...
        for (unsigned long long q = 0; q < chunk_size; q++) {
            for (unsigned long long idx = 0; idx < lags.size(); idx++) {
                p_out[2 * (idx * _nx * ny + i * chunk_size + q)]
                    = workspace[2 * (q * nt + lags[idx])];
            }
            p_out[2 * (lags.size() * _nx * ny + i * chunk_size + q)]
                = 0.5 * tmp[q] / (double)length;
            p_out[2 * ((lags.size() + 1) * _nx * ny + i * chunk_size + q)]
                = p_out[2 * (lags.size() * _nx * ny + i * chunk_size + q)] - tmpAvg[q];
        }
    }
...
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<uint8_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<int16_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<uint16_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<int32_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<uint32_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<int64_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<uint64_t, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<float, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
                            ...
                            py::array_t<Scalar, py::array::c_style> window);
template py::array_t<Scalar>
    PYBIND11_EXPORT ddm_fft(py::array_t<double, py::array::c_style> img_seq,
                            vector<unsigned int> lags,
                            unsigned long long nx,
                            unsigned long long ny,
                            unsigned long long nt,
